...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
	...
	<form name="frm" action="#" method="post">
		...
		<div class="board_act">
			...
			<c:if test="${(bean.data.notice_yn!='Y' || session_rate==10) && (session_userid !='' && session_userid != null && session_userid !='null')}">
			<a href="#" onclick="pwChk('modify'); return false;">수정</a>
			<a href="#" onclick="pwChk('delete'); return false;">삭제</a>
			</c:if>
			<c:if test="${bean.data.notice_yn != 'Y' && session_bsid != 0 && session_bsid != '' }">
