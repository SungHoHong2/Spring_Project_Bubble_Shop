...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("[name='sel_addr']").change(function(){
			
			alert($("[name='sel_addr']:checked").val());
			//${bean.mem_address }
		...
		zipcode = function()
		{
			window.open("/shop/resources/zipcode_sch2.jsp?post1=zip1&post2=zip2&addr1=bg_address&addr2=bg_address2", "pop_zipcode", "width=600, height=490, toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, left=300, top=200")
		}
...
<c:if test="${bean.prev_page == 'pay'}">
	<input type="hidden" id="g_code" name="g_code" value="${bean.a_g_code }" />
</c:if>
...
<input type="hidden" name="a_goods_id" value="${bean.g_code }" />
...
<input type="hidden" id="prev_page" name="prev_page" value="${bean.prev_page}" />

<!-- 총 결제금액, 할인금액, 배송비, 결제예정금액 -->
<br />
<input type="hidden" id="pay_pay1" name="pay_pay1" value="${bean.bb_price }" />
<input type="hidden" id="pay_pay2" name="pay_pay2" value="${bean.bc_price }" />
<input type="hidden" id="pay_pay3" name="pay_pay3" value="2500" />
<input type="hidden" id="pay_pay4" name="pay_pay4" value="0" />

<input type="hidden" id="bc_id" name="bc_id" value="${bean.bc_id }" />
<input type="hidden" id="bc_price" name="bc_price" value="${bean.bc_price }" />
			                ...
			                <td>
			                    <div class="chk_place">
			                        ...
			                        <div class="recent_box">
			                        	<c:forEach items="${bean.add_post }" var="list" varStatus="no">
			                            	<input type="radio" id="ss3" name="sel_addr" class="rdo" value="${list.mem_address }"><label for="ss3">최근 배송지${no.index + 1 }</label>&nbsp;&nbsp;
			                            </c:forEach>
			                        </div>
			                    </div>
			                </td>
