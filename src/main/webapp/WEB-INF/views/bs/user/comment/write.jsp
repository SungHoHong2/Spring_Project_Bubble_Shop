...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if('${session_bsid}'=='' || '${session_bsid}'=='0')
			{
				frm.page.value=1;
				frm.action = 'list';
				frm.submit();
			...
			writeok_go = function()
			{
				...
				if (form_chk(frm, ""))
				{
					/* if(frm.bs_id.value==null || frm.bs_id.value==''|| frm.bs_id.value=='null')
					{	frm.bs_id.value=0;
	...
	<form name="frm" action="writeOk" method="post" enctype="multipart/form-data">
		...
		<%-- <c:choose>
			<c:when test="${session_bsid!=null} ">
				<input type="text" name="bs_id" value="${session_bsid }">
			</c:when>
			<c:otherwise>
